...
<body>
    ...
    <script>
        ...
        async function loadIcebergData() {
            try {
                // Try to load from API endpoints first (for deployed version)
                let latestData;
                let summaryData;
                ...
                try {
                    ...
                    if (!isLocal) {
                        // Try GitHub raw API endpoints (for deployed version)
                        console.log('🌐 Loading from GitHub API endpoints...');
                        const [summaryResponse, latestResponse] = await Promise.all([
                            fetch('https://raw.githubusercontent.com/Joel-hanson/Iceberg-locations/main/api/summary.json'),
                            fetch('https://raw.githubusercontent.com/Joel-hanson/Iceberg-locations/main/api/latest.json')
                        ]);
                    ...
                    latestData = {
                        ...
                        total_icebergs: icebergs.length,
                        icebergs: icebergs
                    };
                    
                    summaryData = {
                        unique_icebergs: icebergs.length,
                        total_records: Object.values(data).reduce((sum, icebergs) => sum + icebergs.length, 0),
                        date_range: { latest: latestDate }
                    };
                    
                    console.log('Fallback data loaded successfully');
                }
                
                // Use the loaded data (either from API or fallback)
                const icebergs = latestData.icebergs || [];
                const latestDate = latestData.last_updated;
...
def generate_api():
    """Generate API endpoints for external data access."""
    print("Generating API endpoints...")
    try:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        api_script = os.path.join(script_dir, "api_generator.py")

        result = subprocess.run(
            [sys.executable, api_script], capture_output=True, text=True
        )
